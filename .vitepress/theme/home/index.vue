    ...
    <div class="next-content">
      <div class="next-content-section" v-for="(nextDate, sIdx) in nextDates" :key="sIdx">
        <div class="next-content-row">
          <!-- 左侧 -->
          <div class="next-content-col">
            <div
              class="next-content-box"
              v-for="(box, bIdx) in nextDate.nextDateLeft"
              :key="bIdx"
            >
              <template v-if="bIdx === 0">
                <div class="next-content-icon">
                  <img :src="box.icon" alt="icon" />
                </div>
                <div class="next-content-title">{{ box.title }}</div>
                <div class="next-content-desc">{{ box.desc }}</div>
              </template>
              <template v-else>
                <div class="next-content-title">{{ box.title }}</div>
                <ul class="next-content-list">
                  <li v-for="(link, i) in box.links" :key="i">
          ...
          <div class="next-content-col">
            <div
              class="next-content-box"
              v-for="(box, bIdx) in nextDate.nextDateRight"
              :key="bIdx"
            >
              <template v-if="bIdx === 0">
                <div class="next-content-title">{{ box.title }}</div>
                <div class="next-content-desc next-content-desc-right">{{ box.desc }}</div>
                <ul class="next-content-list bg-list">
                  <li v-for="(link, i) in box.links" :key="i">
        ...
        <!-- 分割线，最后一个section不显示 -->
        <div
          v-if="sIdx < nextDates.length - 1"
          class="next-content-divider"
        ></div>
      </div>
    </div>
  </div>
  ...
  },
  {
    nextDateLeft: [
      ...
      },
      {
        title: '组件',
        links:[
            { text: 'Container 容器', url: '/tiny-robot/guide/container' },
            { text: 'Bubble 气泡', url: '/tiny-robot/guide/bubble' },
            { text: 'Sender 消息输入框', url: '/tiny-robot/guide/sender' },
            { text: 'Prompts 提示集', url: '/tiny-robot/guide/prompts' },
            { text: 'Welcome 欢迎', url: '/tiny-robot/guide/welcome' },
            { text: 'Feedback 气泡反馈', url: '/tiny-robot/guide/feedback' },
            { text: 'History 历史', url: '/tiny-robot/guide/history' },
            { text: 'DropdownMenu 下拉菜单', url: '/tiny-robot/guide/dropdown-menu' },
            { text: 'SuggestionPopover 建议弹出框', url: '/tiny-robot/guide/suggestion-popover' },
            { text: 'SuggestionPills 建议按钮组', url: '/tiny-robot/guide/suggestion-pills' },
            { text: 'DragOverlay 拖拽浮层', url: '/tiny-robot/guide/drag-overlay' },
            { text: 'Attachments 附件卡片', url: '/tiny-robot/guide/attachments' },
            { text: 'McpServerPicker 插件选择器', url: '/tiny-robot/guide/mcp-server-picker' },
            { text: 'Theme 主题', url: '/tiny-robot/guide/theme' },
          ],
      },
    ...
    nextDateRight: [
      ...
      },
      {
        title: '工具',
        links: [
            { text: 'AI模型交互工具类', url: '/tiny-robot/guide/ai-client' },
            { text: '消息数据管理', url: '/tiny-robot/guide/message' },
            { text: '会话数据管理', url: '/tiny-robot/guide/conversation' },
        ],
      },
      {
        title: '演示',
...
.next-content-list li {
  margin-top: 16px;
}
.next-content-list li:first-child {
  margin-top: 0;
}
.next-content-list a {
